...
subject to PermissionRecolte {f in FORETS, j in JOURS}: # Interdiction de coupe sur une base journalière
    entrer_secteur[f, j] <= Droit_de_coupe[f, j];

subject to EntreeSecteur {f in FORETS, j in JOURS, e in ESSENCES}: # On ne peut pas couper dans un secteur si on n'a pas entré le secteur
